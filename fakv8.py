...
SAVE_DIR = "model/fakv8"
os.makedirs(SAVE_DIR, exist_ok=True)  # Crée le dossier 'model/fakv8' s'il n'existe pas
...
reduce_lr = tf.keras.callbacks.ReduceLROnPlateau(
    monitor='val_loss',
    factor=0.5, #0.2 de base
    patience=4, #3 de base
    min_lr=1e-6,
    verbose=1
)

# 3. Sauvegarde UNIQUEMENT du meilleur modèle (au format .h5 ou .keras)
checkpoint_path = os.path.join(SAVE_DIR, "fakv8_best_model.h5")
...
final_save_path = os.path.join(SAVE_DIR, "fakv8_model_final_best.h5")
